...
class JsonExpressionCodegen {
    ...
    onGet(expr) {
        ...
        if (path instanceof Literal) {
            ...
            const fn = (0, find_1.$$find)((0, json_pointer_1.toPath)(path.val));
    ...
    onDefined(expr) {
        ...
        const fn = (0, find_1.$$find)((0, json_pointer_1.toPath)(pointer));
        const d = this.codegen.addConstant(fn);
    ...
    onCat(expr) {
        ...
        if (areAllLiteral)
            return new Literal(expressions.map((expr) => (0, util_1.str)(expr.val)).join(''));
        this.codegen.link('str');
    ...
    onMin(expr) {
        ...
        if (allLiterals)
            return new Literal((0, util_1.num)(Math.min(...expressions.map((expr) => expr.val))));
    ...
    onMax(expr) {
        ...
        const allLiterals = expressions.every((expr) => expr instanceof Literal);
        if (allLiterals)
            return new Literal((0, util_1.num)(Math.max(...expressions.map((expr) => expr.val))));
        const params = expressions.map((expr) => `${expr}`);
    ...
    onPlus(expr) {
        ...
        const allLiterals = expressions.every((expr) => expr instanceof Literal);
        if (allLiterals)
            return new Literal(expressions.reduce((a, b) => a + (0, util_1.num)(b.val), 0));
    ...
    onMinus(expr) {
        ...
        const allLiterals = expressions.every((expr) => expr instanceof Literal);
        if (allLiterals)
            return new Literal(expressions.slice(1).reduce((a, b) => a - (0, util_1.num)(b.val), (0, util_1.num)(expressions[0].val)));
        const params = expressions.map((expr) => `(+(${expr})||0)`);
        return new Expression(`${params.join(' - ')}`);
    }
    onAsterisk(expr) {
        ...
        const allLiterals = expressions.every((expr) => expr instanceof Literal);
        if (allLiterals)
            return new Literal(expressions.reduce((a, b) => a * (0, util_1.num)(b.val), 1));
        const params = expressions.map((expr) => `(+(${expr})||0)`);
        return new Expression(`${params.join(' * ')}`);
    }
    onSlash(expr) {
