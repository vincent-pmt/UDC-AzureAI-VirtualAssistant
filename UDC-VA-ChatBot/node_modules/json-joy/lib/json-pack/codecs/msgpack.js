"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.MsgPackJsonValueCodec = void 0;
var msgpack_1 = require("../msgpack");
var MsgPackDecoder_1 = require("../msgpack/MsgPackDecoder");
var MsgPackJsonValueCodec = (function () {
    function MsgPackJsonValueCodec(writer) {
        this.id = 'msgpack';
        this.format = 1;
        this.encoder = new msgpack_1.MsgPackEncoder(writer);
        this.decoder = new MsgPackDecoder_1.MsgPackDecoder();
    }
    return MsgPackJsonValueCodec;
}());
exports.MsgPackJsonValueCodec = MsgPackJsonValueCodec;
