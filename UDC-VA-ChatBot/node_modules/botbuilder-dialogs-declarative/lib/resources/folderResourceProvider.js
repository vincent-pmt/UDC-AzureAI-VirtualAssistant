...
class FolderResourceProvider extends resourceProvider_1.ResourceProvider {
    ...
    constructor(resourceExplorer, folder, includeSubFolders = true, monitorChanges = true) {
        ...
        folder = (0, path_1.normalize)(folder);
        this.directory = folder;
        this._id = this.directory;
        this.refresh();
        if (monitorChanges) {
            this._watcher = (0, chokidar_1.watch)(folder, { depth: includeSubFolders ? undefined : 1 });
            this._watcher
    ...
    refresh() {
        ...
        const filteredFiles = files.filter((filename) => this.resourceExplorer.resourceTypes.has((0, path_1.extname)(filename).toLowerCase().replace(/^\./, '')));
    ...
    onResourceAdded(path) {
        const ext = (0, path_1.extname)(path.toLowerCase()).replace(/^\./, '');
